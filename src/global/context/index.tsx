...
const ContextWrapper = ({ children }: { children: ReactNode }) => {
    ...
    async function fetchApiForAllCartItems() {
        if (userData) {
            let res = await fetch(`/api/cartfunction?user_id=${userData.uuid}`);
            if (!res.ok) {
    ...
    async function dispatch(payload: string, data: any) {
        if (payload === "addToCart") {
            await fetch(`/api/cartfunction`, {
                method: "POST",
                body: JSON.stringify(data),
            });
        } else if (payload === "removeFromCart") {
            let dataa = await fetch(`/api/cartfunction?product_id=${data.product_id}&user_id=${data.user_id}`, {
                method: "DELETE",
            });
            let NotData = await dataa.json();
        } else if (payload === "updateCart") {
            setLoading(true);
            let dataa = await fetch(`/api/cartfunction`, {
                method: "PUT",
                body: JSON.stringify(data)
            });
            let NotData = await dataa.json();
    ...
    function signUpUser(email: string, password: string) {
        ...
        .catch((res: any) => {
            ...
            console.log("sign up error is here", error)
            setLoading(false);
        });
        setLoading(false);
    };
    ...
    function updateUserNamePhoto(userName: string, photoURL?: string) {
        ...
        if (user) {
            updateProfile(user, {
                displayName: userName, photoURL: "https://freesvg.org/img/abstract-user-flat-4.png"
            }).then(() => {
                setLoading(false);
                window.location.reload();
            }).catch((error: any) => {
